    ...
    for epoch in range(n_epoch):  # epoch 수 만큼 반복
        ...
        while not game_over:  # 게임이 끝날 때 까지
            ...
            h = model.fit(
                inputs,
                targets,
                epochs=8,  # 학습 데이터 전체셋을 몇 번 학습하는지를 의미합니다. 동일한 학습 데이터라고 하더라도 여러 번 학습할 수록 학습 효과는 커집니다. 8
                # 하지만, 너무 많이 했을 경우 모델의 가중치가 학습 데이터에 지나치게 최적화되는 과적합(Overfitting) 현상이 발생합니다.
                batch_size=16,  # 만약 batch_size가 10이라면  , 총 10개의 데이터를 학습한 다음 가중치를 1번 갱신하게 됩니다. 16
